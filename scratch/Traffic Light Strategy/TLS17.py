...
while True:
    ...
    if os.path.isfile("NIFTY BANK.csv"):
        data1 = pd.read_csv('NIFTY BANK.csv', index_col=0)
        data = data1.iloc[:-1 , :]
    ...
    #pdb.set_trace()
       ...
       #  os.system('get_data.py')
       # # Get the last row's time value
       #  starttime = pd.to_datetime(data1.iloc[-1]['Date']).time()

       #  # Get the current time without microseconds
       #  time_now = dt.datetime.now().time().replace(microsecond=0)

       #  # Calculate the time difference
       #  time_diff = dt.datetime.combine(dt.date.today(), time_now) - dt.datetime.combine(dt.date.today(), starttime)

       #  # Check if the time difference is less than 1 minute
       #  if_diff.total_seconds() < 60:
       #      print("The time difference is less than 1 minute.")
       #  else:
       #     print("The time difference is greater than or equal to 1 minute.")
       #     os.system('get_data.py')
